...
func GetCfToolEnv(ctx context.Context, token, dominio, tenant, coreApiVersion string, monolith bool) (TenantEnv, error) {
	...
	// Log di TUTTI i parametri passati ad ApiCallGET
	Logga(ctx, os.Getenv("JsonLog"), fmt.Sprintf(
		"Calling ApiCallGET with: args=%+v, service=%s, path=%s, token=%s, dominio=%s, coreApiVersion=%s",
	...
	if envRes.BodyJson == nil {
    		Logga(ctx, os.Getenv("JsonLog"), "BodyJson è nil")
	} else {
    		Logga(ctx, os.Getenv("JsonLog"), fmt.Sprintf("BodyJson type: %T, value: %+v", envRes.BodyJson, envRes.BodyJson))
	}
